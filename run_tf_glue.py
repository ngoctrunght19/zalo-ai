...
valid_dataset = glue_convert_examples_to_features(
    data["validation"], tokenizer, 128, TASK
)
...
if TASK == "mrpc":
    ...
    inputs_1 = tokenizer.encode_plus(
        sentence_0, sentence_1, add_special_tokens=True, return_tensors="pt"
    )
    inputs_2 = tokenizer.encode_plus(
        sentence_0, sentence_2, add_special_tokens=True, return_tensors="pt"
    )
    ...
    print(
        "sentence_1 is",
        "a paraphrase" if pred_1 else "not a paraphrase",
        "of sentence_0",
    )
    print(
        "sentence_2 is",
        "a paraphrase" if pred_2 else "not a paraphrase",
        "of sentence_0",
    )
